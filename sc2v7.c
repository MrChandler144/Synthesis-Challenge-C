...
// v7 is a general case for 5 gears: a1 a2 a3, b1 b2			(bad)
...
int main(void)
{
	...
	for (a=2; a<17; a=a+2)
	{
		for (b=2; b<17; b=b+2)
		{
			for (a1=1; a1<8; a1=a1+0.02)
			{
				...
				{
					for (b2=1; b2<8; b2=b2+0.02)
					{
						a2=a-a1;
						a3=b-a2;
						c=b1+b2;
						distance=a+b+c;
						reductionRatio = (a3*b2)/(a1*b1);
						...
						if ((distance > 25) && (distance < 27) && (reductionRatio >= 11.88) && (reductionRatio <= 12.12))
						{
							...
							score = 0.2*(pow(a1,2) + pow(a2,2) + pow(a3,2) + pow(b1,2) + pow(b2,2));
	...
	printf("\noh yeah\n");
	printf("score is %f\n", scoreRecord);
	printf("%f %f %f %f %f \n", a1Record, a2Record, a3Record, b1Record, b2Record);
